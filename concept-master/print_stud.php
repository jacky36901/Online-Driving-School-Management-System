		...
		if(isset($_POST['search']))
{
    ...
    $query="select * from add_trainer where trainer_id='$tran'";
...
<div style="font-size:25px;align:center;">
    ...
    <?php
        $rr="select * from add_trainer where trainer_id='$tran'";
                $q=mysqli_query($con,$rr);
                $res=mysqli_fetch_array($q);
        
    echo "<div style='font-size:25px;'>Trainer Name:".$res['name']."</div>";
     ...
     <table border="1" width=100%>
            ...
            $query4="select distinct stud_id from add_slot where trainer_id='$op'and status1=2 and date between'$txtStartDate'and'$txtEndDate' order by date";
            $tbl4=mysqli_query($con,$query4);
            while($fetch4=mysqli_fetch_array($tbl4))
            {
                $op3=$fetch4['stud_id'];
            $query2="select * from registration where username='$op3'";
            $query3="select * from stud_reg where username='$op3' and school_id='$op2'";
    $tbl2=mysqli_query($con,$query2);
            $tbl3=mysqli_query($con,$query3);
            while(($fetch2=mysqli_fetch_array($tbl2))&&($fetch3=mysqli_fetch_array($tbl3)))
            {
                                                ?>
					<tr><td><?php echo $fetch2['name']?></td><td><?php echo $fetch3['dob']?></td><td><?php echo $fetch3['gender'] ?></td><td><?php echo $fetch2['phone'] ?></td><td><?php echo $fetch2['hname'] ?></td><td><?php echo $fetch2['place'] ?></td><td><?php echo $fetch2['dis'] ?></td><td><?php echo $fetch2['pin'] ?></td><td><?php echo $fetch2['email'] ?></td><td><?php echo $fetch2['jdate'] ?></td></tr>
