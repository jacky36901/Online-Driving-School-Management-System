		...
		if(isset($_POST['search']))
{
    ...
    $query="select * from stud_reg where school_id='$user'";
...
$op=$fetch['username'];
        echo $op;
            $query1="select * registration where username='$op'and jdate between'$txtStartDate'and'$txtEndDate' order by jdate";
            $tbl2=mysqli_query($con,$query1);
            while($res=mysqli_fetch_array($tbl2))
            {
                $op2=$fetch4['username'];
                echo $op2;
                                                ?>
					<tr><td><?php echo $fetch['name']?></td><td><?php echo $fetch['dob']?></td><td><?php echo $fetch['gender'] ?></td><td><?php echo $res['phone'] ?></td><td><?php echo $res['hname'] ?></td><td><?php echo $res['place'] ?></td><td><?php echo $res['dis'] ?></td><td><?php echo $res['pin'] ?></td><td><?php echo $res['email'] ?></td><td><?php echo $res['jdate'] ?></td></tr>
