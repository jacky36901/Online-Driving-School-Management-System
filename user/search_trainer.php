...
<center><h1>TRAINER REPORT</h1></center>
    
    <div style="position:relative;left:220px;top:0px;border:groove;width:500px;height:200px;" >
    <form method="post" action="print_trainer.php">
        
		<table style="position:relative;top:20px;left:40px;height:100px;width:400px;">
            <tr><td><font color="red">SCHOOL NAME:</font> </td>
		<td><div class="form-select">
                                    <select name="train" id="train">
                                        <option value=""></option>
                                         <?php
                                        $sq3="select * from school_reg";
                                    $me2=mysqli_query($con,$sq3);

    while($fetch=mysqli_fetch_array($me2))
						{
                            ?>
               <option value="<?php echo $fetch['username']?>"><?php echo $fetch['name']?>  <?php
                        }
?></option>>
                    </select></div></td>
		</tr>
		...
		<td> <input type="date" name="txtEndDate"></td>
            ...
            <td></td></tr>
      <tr>
		<td colspan="2"><center><input type="submit" name="search" id="search" value="search"></center></td>
		</tr>
       </table>
		</form>
		</div> 
